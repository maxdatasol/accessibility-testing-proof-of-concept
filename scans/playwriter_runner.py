...
# ----------------------------
# PDF Reporting
# (same as Selenium, reuse AuditPDF class)
# ----------------------------
...
# Playwright scan
# ----------------------------
async def scan_url(url, guidance, output_dir):
    async with async_playwright() as p:
        ...
        # axe-core injection
        # Using axe-playwright requires Node.js integration or script injection
        axe_script = "https://cdnjs.cloudflare.com/ajax/libs/axe-core/4.8.3/axe.min.js"
        await page.add_script_tag(url=axe_script)
        results = await page.evaluate("""async () => { return await axe.run(); }""")
        
        flattened = flatten_violations(results.get("violations", []))
        ...
        pdf = AuditPDF()
        pdf.add_page()
        pdf.add_summary(url, total_violations=len(flattened), lighthouse_score=lh_score)
        ...
        await browser.close()
        print(f"Completed Playwright scan for {url}. Report saved: {pdf_file}")
...
async def main(urls):
    ...
    for url in urls:
        try:
            await scan_url(url, guidance, output_dir)
        except Exception as e:
            print(f"Error scanning {url}: {e}")

if __name__ == "__main__":
    urls = ["https://example.com", "http://max-data.com"]
